...
const OrderTrackingPage = () => {
  ...
  const orders = data as UserOrders[]; // ✅ Cast once here
      ...
      orders.forEach((order) => {
        order.items.forEach((item: OrderItem) => {
          if (item.product.imageUrls && item.product.imageUrls[0]?.data) {
            const IMG = new Blob(
              [new Uint8Array(item.product.imageUrls[0].data)],
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white shadow-lg rounded-lg">
      ...
      <p className="text-center text-gray-600 mb-6">
        Here are your recent orders along with their current status.
      </p>
      ...
      {orders.length > 0 ? (
        orders.map((order) => (
          <div key={order._id} className="mt-8 p-4 border border-gray-300 rounded-lg">
            ...
            <p className="text-gray-600">
              Paid: <span className="font-medium">{order.isPaid ? "Yes" : "No"}</span>
            </p>
